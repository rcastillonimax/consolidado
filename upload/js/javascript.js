$(document).ready(function() {
	...
	$('#drop-files').bind('drop', function(e) {
		...
		$.each(files, function(index, file) {
			...
			if (!files[index].type===('application/octet-stream')
            ||!files[index].type==('application/txt')
            ||!files[index].type==('text/anytex')
            ||!files[index].type==('application/octet-stream')
            ) 
            {
				
				if(errMessage == 0) {
					$('#drop-files').html('Solo archivos .dat_1');
					++errMessage
				}
				else if(errMessage == 1) {
					$('#drop-files').html('Solo archivos .dat_2');
					++errMessage
				}
				else if(errMessage == 2) {
					$('#drop-files').html('Solo archivos .dat_3');
					++errMessage
				}
